<template>
  <teleport to="body">
    <div 
      v-if="isOpen" 
      class="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-4 bg-slate-950/70 backdrop-blur-xs animate-in fade-in duration-200"
      @click.self="handleClose"
    >
      <div 
        class="bg-white text-slate-900 w-full sm:max-w-2xl h-full sm:h-auto sm:max-h-[90vh] sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden border border-slate-200/80 animate-in zoom-in-95 duration-200"
      >
        <!-- Modal Header -->
        <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/80 shrink-0">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="font-extrabold text-base sm:text-lg text-slate-900 tracking-tight flex items-center gap-2">
          ...
          <button 
            @click="handleClose"
            class="w-8 h-8 rounded-full bg-slate-200/70 hover:bg-slate-200 text-slate-500 flex items-center justify-center transition cursor-pointer"
        ...
        <div class="p-4 sm:p-6 overflow-y-auto flex-1 space-y-5">
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div>
                <div class="font-bold flex items-center gap-1.5 text-emerald-950">
                  <span>Perangkat Ini Adalah Perangkat Utama (Main Device)</span>
            ...
            <button 
              ...
              class="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-full text-xs transition flex items-center gap-1.5 shrink-0 cursor-pointer shadow-xs active:scale-95"
            >
          ...
          <div 
            ...
            class="p-4 rounded-2xl bg-blue-50/80 border border-blue-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs text-blue-900"
          >
            <div class="flex items-start gap-3">
              <div class="w-8 h-8 rounded-xl bg-blue-100 text-blue-700 flex items-center justify-center shrink-0 border border-blue-200">
                <ShieldAlert class="w-4 h-4" />
              </div>
              <div>
                <div class="font-bold text-blue-950">Perangkat Ini Belum Menjadi Perangkat Utama</div>
            ...
            <button 
              ...
              class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-full text-xs transition flex items-center gap-1.5 shrink-0 cursor-pointer shadow-xs active:scale-95"
            >
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div>
                <div class="font-bold text-amber-950">Belum Ada Perangkat Utama Ditetapkan</div>
            ...
            <button 
              @click="openMakeMainModal"
              class="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-full text-xs transition flex items-center gap-1.5 shrink-0 cursor-pointer shadow-xs active:scale-95"
            >
          ...
          <div>
            <div class="flex items-center justify-between mb-3">
              <div class="flex items-center gap-2">
                <h3 class="font-extrabold text-xs text-slate-700 uppercase tracking-wider">
              ...
              <button 
                ...
                class="text-[11px] text-blue-600 hover:text-blue-700 font-semibold flex items-center gap-1 cursor-pointer"
            ...
            <div class="space-y-2.5">
              ...
              >
                ...
                <div class="flex items-start gap-3.5">
                  ...
                  >
                    <Smartphone v-if="session.deviceType === 'mobile'" class="w-5 h-5" />
                    <Tablet v-else-if="session.deviceType === 'tablet'" class="w-5 h-5" />
                    <Laptop v-else class="w-5 h-5" />
                  </div>

                  <div class="space-y-1">
                    <div class="flex flex-wrap items-center gap-2">
                      <span class="font-bold text-sm text-slate-900">{{ session.deviceName }}</span>
                      
                      <!-- Badge Perangkat Ini -->
                      <span 
                        v-if="session.isCurrentDevice"
                        class="px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-blue-100 text-blue-800 border border-blue-200 flex items-center gap-1"
                      >
                      ...
                      <span 
                        ...
                        class="px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-emerald-100 text-emerald-800 border border-emerald-200 flex items-center gap-1"
                      >
                    ...
                    <div class="text-[11px] text-slate-500 flex flex-wrap items-center gap-y-1 gap-x-3">
          ...
          <div class="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 text-slate-600 text-[11px] space-y-1">
        ...
        <!-- Modal Footer -->
        <div class="px-5 py-3.5 border-t border-slate-100 bg-slate-50/80 flex items-center justify-end gap-2 shrink-0">
          <button 
            @click="handleClose"
            class="px-5 py-2.5 bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold rounded-full text-xs transition cursor-pointer"
          >
            Tutup
          </button>
        </div>
    ...
    <div 
      ...
      class="fixed inset-0 z-60 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs animate-in fade-in duration-200"
      ...
      >
        <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-blue-50/50">
          <div class="flex items-center gap-2.5">
            ...
            <div>
              <h3 class="font-bold text-sm text-slate-900">Verifikasi Perangkat Utama</h3>
              <p class="text-[11px] text-slate-500">Kirim kode verifikasi ke email</p>
            </div>
          </div>
          <button 
            @click="isVerifyModalOpen = false"
            class="w-7 h-7 rounded-full bg-slate-200/70 hover:bg-slate-200 text-slate-500 flex items-center justify-center transition cursor-pointer"
          >
            <X class="w-3.5 h-3.5" />
          </button>
        </div>
        ...
        <div class="p-5 space-y-4">
          ...
          <div class="text-xs text-slate-600 space-y-2">
            ...
            <div class="p-3 bg-slate-50 rounded-xl border border-slate-200 font-mono font-bold text-slate-800 text-center text-xs">
              {{ maskedEmail }}
            </div>
          </div>
          ...
          <div 
            ...
            class="p-3 rounded-xl bg-amber-50 border border-amber-200 text-amber-900 text-xs flex items-center justify-between"
          >
            <span>Kode pengujian (simulasi): <strong>{{ simulatedCodeNotice }}</strong></span>
            <button 
              @click="otpCode = simulatedCodeNotice"
              class="px-2 py-1 bg-amber-200 hover:bg-amber-300 rounded-lg text-[10px] font-bold text-amber-900 cursor-pointer"
            >
              Isi Otomatis
            </button>
          </div>
        ...
        <div class="px-5 py-3.5 border-t border-slate-100 bg-slate-50 flex items-center justify-end gap-2">
          ...
          <button 
            ...
            class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-full text-xs transition flex items-center gap-1.5 cursor-pointer shadow-md shadow-blue-200 disabled:opacity-50 disabled:cursor-not-allowed active:scale-95"
          >
            <Crown class="w-3.5 h-3.5 text-amber-300" />
      ...
      >
        <div class="p-6 text-center space-y-4">
          ...
          <div>
            ...
            <p class="text-xs text-slate-500 mt-1">
              Apakah Anda yakin ingin mencabut sesi login pada <strong class="text-slate-800">{{ revokeTargetSession.deviceName }}</strong>? Pengguna pada perangkat tersebut akan langsung dikeluarkan (logout) dari aplikasi.
            </p>
          </div>
        </div>
        ...
        <div class="px-5 py-3.5 border-t border-slate-100 bg-slate-50 flex items-center justify-end gap-2">
          ...
          <button 
            ...
            :disabled="isRevoking"
            class="px-5 py-2.5 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-full text-xs transition flex items-center gap-1.5 cursor-pointer shadow-md shadow-rose-200 disabled:opacity-50 active:scale-95"
          >
